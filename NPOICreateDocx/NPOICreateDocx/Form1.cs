...
/*
 * 本例子提供的NPOI是tonyqus提供的2.0.9.0源码经过修改编译。
 * 例中包括：
 * 1、docx创建和存储
 * 2、页面设置
 * 3、段落创建、设置
...
namespace NPOICreateDocx
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            XWPFDocument m_Docx = new XWPFDocument();//创建文档示例
            //页面设置
            //A4:W=11906,h=16838
            ...
            m_p.AddNewPPr().AddNewSpacing().line = "400";//固定20磅
            ...
            //m_CTTblPr.tblpPr.tblpXSpec = ST_XAlign.left;//tblpXSpec优先tblpX；若tblpXSpec有值，表由tblpXSpec定位
            //m_CTTblPr.tblpPr.tblpYSpec = ST_YAlign.top;//tblpYSpec优先tblpY；若tblpYSpec有值，表由tblpYSpec定位  
            ...
            gr.AddPicture(gfs, (int)PictureType.PNG, "1.png", 500000, 500000, posH, posV, wrapSquare, an, simplePos, effectExtent);
            gfs.Close();
            //wrapTight（紧密）
            ...
            gr.AddPicture(gfs, (int)PictureType.PNG, "1.png", 500000, 500000, posH, posV,wrapTight, an, simplePos,effectExtent);
            gfs.Close();
            //wrapThrough(穿越)
            ...
            gr.AddPicture(gfs, (int)PictureType.PNG, "15.png", 500000, 500000, posH, posV, wrapThrough, an, simplePos, effectExtent);
            gfs.Close();
            ...
            m_p.AddNewR().AddNewT().Value = "NPOI";
            //书签0结束
            ...
            //gp.IndentationFirstLine = Indentation("宋体", 21, 2, FontStyle.Regular);//段首行缩进2字符
            //gr = gp.CreateRun();
            m_p.AddNewR().AddNewT().Value = "是POI项目的.NET版本。POI是一个开源的Java读写Excel、WORD等微软OLE2组件文档的项目。使用NPOI你就可以在没有安装Office或者相应环境的机器上对WORD/EXCEL文档进行读写。NPOI是构建在POI3.x版本之上的，它可以在没有安装Office的情况下对Word/Excel文档进行读写";

            //书签1开始
            m_bookId = (m_p.Items.Count - 1) / 2;
            m_ctbook1 = new CT_Bookmark();
            m_ctbook1.id = m_bookId.ToString(); //"1";
            m_ctbook1.name = "NPOI2";
            m_p.Items.Add(m_ctbook1);
            m_p.ItemsElementName.Add(ParagraphItemsChoiceType.bookmarkStart);
            m_p.AddNewR().AddNewT().Value = "操作";
            //书签1结束
            m_ctbook1 = new CT_Bookmark();
            m_ctbook1.id = m_bookId.ToString(); //"1";
            m_p.Items.Add(m_ctbook1);
            ...
            m_bookId = 0;
            m_p.Items = new System.Collections.ArrayList();
            CT_Bookmark m_ctbook = new CT_Bookmark();
            m_bookId = m_p.Items.Count;
            m_ctbook.id = m_bookId.ToString(); //"0";
            m_ctbook.name = "NPOI3";
            m_p.Items.Add(m_ctbook);
            m_p.ItemsElementName = new List<ParagraphItemsChoiceType>();
            m_p.ItemsElementName.Add(ParagraphItemsChoiceType.bookmarkStart);
            m_p.AddNewR().AddNewT().Value = "NPOI";
            //书签2结束
            m_ctbook = new CT_Bookmark();
            m_ctbook.id = m_bookId.ToString();//"0";
            m_p.Items.Add(m_ctbook);
            m_p.ItemsElementName.Add(ParagraphItemsChoiceType.bookmarkEnd);
            ...
            m_bookId = (m_p.Items.Count - 1) / 2;
            m_ctbook = new CT_Bookmark();
            m_ctbook.id = m_bookId.ToString(); //"1";
            m_ctbook.name = "NPOI4";
            m_p.Items.Add(m_ctbook);
            m_p.ItemsElementName.Add(ParagraphItemsChoiceType.bookmarkStart);
            m_p.AddNewR().AddNewT().Value = "测试";
            //书签3结束
            m_ctbook = new CT_Bookmark();
            m_ctbook.id = m_bookId.ToString(); //"1";
            m_p.Items.Add(m_ctbook);
            ...
            //gr.SetText("NPOI");
            gr = gp.CreateRun();
            gr.SetText("之所以强大，并不是因为它支持导出Excel，而是因为它支持导入Excel，并能“理解”OLE2文档结构，这也是其他一些Excel读写库比较弱的方面。通常，读入并理解结构远比导出来得复杂，因为导入你必须假设一切情况都是可能的，而生成你只要保证满足你自己需求就可以了，如果把导入需求和生成需求比做两个集合，那么生成需求通常都是导入需求的子集。");
            //在本段中插图-wrapSquare
            //gr = gp.CreateRun();//创建run
            ...
            gfs = new FileStream(m_PicPath + "15.png", FileMode.Open, FileAccess.Read);//读取图片文件
            gr.AddPicture(gfs, (int)PictureType.PNG, "15.png", 500000, 500000, posH, posV, wrapSquare, an, simplePos, effectExtent);
            gfs.Close();
